...
opening_book::OpeningBook g_book;
bool g_bookLoaded = false;
std::mt19937 g_bookRng(0xC0FFEEu);
...
void AppendBookHistory(AppState* app, fatpup::Move move)
{
    if (!app->bookActive)
    {
        return;
    }
    if (app->bookHistoryUci.size() >= static_cast<std::size_t>(opening_book::BookLineLen))
    {
        return;
    }
    const std::string uci = MoveToUci(move);
    app->bookHistoryUci.append(uci, 0, opening_book::UciMoveLen);
}
...
fatpup::Move FindLegalMoveByUci(const fatpup::Position& position, const std::string& uci)
{
    if (uci.size() < 4)
    {
        return fatpup::Move();
    }
    const int srcCol = uci[0] - 'a';
    const int srcRow = uci[1] - '1';
    const int dstCol = uci[2] - 'a';
    const int dstRow = uci[3] - '1';
    const std::vector<fatpup::Move> candidates = position.possibleMoves(srcRow, srcCol, dstRow, dstCol);
    if (candidates.empty())
    {
        return fatpup::Move();
...
fatpup::Move PickBookMove(AppState* app)
{
    if (!g_bookLoaded || !app->bookActive)
    {
        return fatpup::Move();
    }
    if (app->bookHistoryUci.size() >= static_cast<std::size_t>(opening_book::BookLineLen))
    ...
    const std::string bookUci = g_book.sampleNext(app->bookHistoryUci, &g_bookRng);
    if (bookUci.empty())
    {
        return fatpup::Move();
    }
    return FindLegalMoveByUci(app->position, bookUci);
}

fatpup::Move PickEngineMove(AppState* app)
{
    const fatpup::Move bookMove = PickBookMove(app);
    if (!bookMove.isEmpty())
    {
        return bookMove;
    }
...
bool EnsureInitialized(std::string* /*error*/)
{
    ...
    if (!g_bookLoaded)
    {
        g_bookLoaded = g_book.load("/opening_book.txt");
    }
